...
Before running LLM evaluation, you need to process raw EPPI-Reviewer JSON annotations into structured format. The annotation converter script is available at `app/scripts/annotation_converter_cli.py` and can be used to convert raw EPPI-Reviewer data into the structured format needed for LLM evaluation.
