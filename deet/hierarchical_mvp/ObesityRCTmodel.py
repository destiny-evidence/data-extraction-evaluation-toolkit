"""Pydantic data models for Obesity RCT outcome data extraction.

Identical structure to RCTmodel.py in the first instance. Fields are expected
to be adapted later for obesity-specific extraction needs.
"""
...
class Intervention(BaseModel):
    group_name: str = Field(
        description="Identify and extract the name of the intervention group or groups from this study. This could be in the form of abbreviations. For example, mindfulness-based group, beverage-only intervention. "
    )
    ...
    description: str = Field(
        description="Describe this intervention arm. If only some participants received the treatment in this arm, note that. Note the frequency, duration, and amount or dose of the intervention. Be as precise as possible: if duration, frequency or dose are mentioned, they need to be in the answer. If there are multiple components to the intervention (e.g. physical activity plus diet), state all of them. Copy entire sections of the text if relevant."
    )
    ...
    no_participants_moved: str = Field(
        description="Identify and extract the number of participants moved to or from this intervention with the reason for the move. Don't extrapolate. Leave blank if not mentioned."
    )
    theoretical_basis: str = Field(
        description="Only name theoretical frameworks that are explicitly used in the development of this intervention. E.g. 'Health Promoting Schools Framework', 'socioecological model', 'social cognitive theory', etc."
    )
    ...
    def to_csv_row(self) -> dict[str, str]:
        return {
            ...
            "no_participants_moved": self.no_participants_moved,
...
class Dichotomous_Outcome(BaseModel):
    ...
    outcome_definition: str = Field(
        description="Provide a definition of the outcome and how it was defined. For example, if it was an adverse event. If a study investigated the effects of physical activity and reports complaints or number of dissatisfied participants, then the answer would include the outcome name and how it was operationalised. Be as precise as possible."
    )
    ...
    measurement_reporting: str = Field(
        description="Extract who measured or reported this outcome. Provide any further detail that is relevant to the person measuring or reporting such as standardisations (i.e. weighed with or without shoes, calculated with an average, training, intra and inter-observer reliability.) Leave blank if not reported."
    )
    statistical_methods_used: str = Field(
        description="Extract all information on the statistical methods for measuring this outcome. Do not extrapolate. If applicable, the answer should consist of general mathematical techniques, tools or tests, such as regression, t-test, ANOVA, chi-square test, etc. You can also specify the software implementations, e.g. 'ANOVA implemented with the SPSS software package'. If no statistical techniques were used, leave blank. Don't mention very basic statistics like averages, standard deviations, etc. If the study includes clusters, identify and extract if the study accounted for clustering (e.g. ICC, adjusted for clustering) and the ICC number."
    )
...
class Continuous_Outcome(BaseModel):
    ...
    unit_of_measurement: str = Field(
        description="Unit of measurement for this outcome, e.g kg, cm, body mass index (BMI), z-score. Leave blank if not reported."
    )
...
class Study_Characteristics(BaseModel):
    ...
    population_description: str = Field(
        description="Identify and extract population characteristics of participants who were approached to participate in this study. If available, provide an estimated number of the people or schools approached. Do not include inclusion or exclusion criteria, and baseline characteristics. Include information regarding people or schools approached."
    )
    setting: str = Field(
        description="If multiple countries or regions, list all of them. You can also say something like 'multinational' if there are multiple countries but individual countries are not specified. Provide answer in bullet points for 'City', 'State' and 'Country' if available. Leave the answer blank if the country is not specified."
    )
    ...
    method_of_recruitment: str = Field(
        description="Identify and extract the method of recruitment of participants. For example, via phone, mail, voluntary.  Note that recruitment methods may differ for schools and students, so report each criterion individually and specify the group it applies to."
    )
    ...
    key_conclusions: str = Field(
        description="What are the key conclusions of the authors of this study?"
    )
    no_participants_moved: str = Field(
        description="Identify and extract the number of participants moved from one group to another along with the reason for the move. Don't extrapolate. Leave blank if not mentioned."
    )
    no_missing_reasons: str = Field(
        description="Identify and extract the number of missing participants, defined as those who withdrew from the study or were excluded between randomization and follow-up. Extract the reasons for missingness if provided (e.g., withdrawal, exclusion after randomization). If missing participants are reported by group, list each group separately and use bullet points. Extract any details on whether missing participants were analysed. Do not extract any information about imputation of missing data. Do not extrapolate; leave blank if no missing participants are reported."
    )
    ...
    def to_csv_row(self) -> dict[str, str]:
        return {
            ...
            "method_of_recruitment": self.method_of_recruitment,
