...
def test_select_attributes_returns_all_when_no_filter(extractor, make_attr):
    attrs = [make_attr(1), make_attr(2)]
    assert extractor._select_attributes(attrs, None) == attrs


def test_select_attributes_filters_to_matching_ids(extractor, make_attr):
    attrs = [make_attr(1), make_attr(2), make_attr(3)]
