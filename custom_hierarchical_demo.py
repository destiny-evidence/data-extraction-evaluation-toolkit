...
Open this file in VS Code or PyCharm and run it cell-by-cell (each `# %%` marks a cell).
Edit the config dicts in place before running their corresponding "write config" cell.
"""
...
# %% Parse PDFs: convert every supported file directly inside this folder to markdown.
...
# %% Single-study config (edit this dict, then run the next cell to write it to disk)
single_study_config = {
    "study_type": "RCT",
...
# %% Batch config (edit this dict, then run the next cell to write it to disk)
batch_config = {
    "study_type": "RCT",
    "llm_model": "anthropic/claude-opus-5",
    "max_tokens": 30000,
