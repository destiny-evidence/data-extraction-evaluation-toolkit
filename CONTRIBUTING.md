...
- We strongly recommend you use [conventional commit](https://www.conventionalcommits.org/en/v1.0.0/) tags. Indeed, if you're using pre-commit hooks, which we also strongly encourage (`pre-commit install`), then you will have a hook which forces you to use them. These tags will lead to your feature potentially incrementing the version number of `deet`. Please keep this in mind when tagging your commits.
